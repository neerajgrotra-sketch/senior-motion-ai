...
function estimateStepSeconds(step: SessionStep): number {
  return (
    step.targetReps * (SECONDS_PER_REP_ESTIMATE + step.targetHoldSeconds) +
    step.restSeconds
  );
}
      ...
      <div style={shellStyle}>
        <section style={heroStyle}>
          ...
          <div style={heroChipRowStyle}>
            ...
            <div style={heroChipStyle}>
              Approx. {formatDuration(totalEstimatedSeconds)}
            </div>
              ...
              {value.steps.length === 0 ? (
                ...
                <div style={stepsListStyle}>
                  {value.steps.map((step, index) => {
                    const exerciseMeta = EXERCISE_OPTIONS.find(
                      ...
                      <article key={step.id} style={stepCardStyle}>
                        ...
                        <div style={controlsGridStyle}>
                          <NumberField
                            ...
                            min={1}
                            onChange={(v) =>
                              updateStep(step.id, { targetReps: v })
                            }
                          />
                          <NumberField
                            label="Hold time"
                            ...
                            suffix="sec"
                            onChange={(v) =>
                              updateStep(step.id, { targetHoldSeconds: v })
                            }
                          />
                          <NumberField
                            label="Rest time"
                            ...
                            suffix="sec"
                            onChange={(v) =>
                              updateStep(step.id, { restSeconds: v })
                            }
                          />
                          <SelectField
                            ...
                            onChange={(v) =>
                              updateStep(step.id, {
                                requiredPosture:
                                  v as SessionStep["requiredPosture"],
                              })
                            }
                          />
                        </div>
            ...
            <div style={stickyColumnStyle}>
              <section style={summaryCardStyle}>
                ...
                <div style={summaryFootnoteStyle}>
                  This launches the guided session runner using the current
                  configuration.
                </div>
              </section>
            </div>
...
const exerciseTileStyle: CSSProperties = {
  textAlign: "left",
  background:
    "linear-gradient(180deg, rgba(15,23,42,0.84) 0%, rgba(10,15,27,0.84) 100%)",
...
const stepCardStyle: CSSProperties = {
  ...
  border: "1px solid rgba(148,163,184,0.14)",
  background:
    "linear-gradient(180deg, rgba(9,16,29,0.82) 0%, rgba(13,21,37,0.82) 100%)",
  padding: 22,
...
const readinessCardStyle: CSSProperties = {
  borderRadius: 20,
  background:
    "linear-gradient(180deg, rgba(8,15,28,0.9) 0%, rgba(10,18,33,0.9) 100%)",
  border: "1px solid rgba(148,163,184,0.14)",
  padding: 18,
  marginBottom: 18,
};
